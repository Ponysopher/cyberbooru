...
async function createManyChunked<T, M extends { createMany(args: { data: T[] }): Promise<any> }>(
  records: T[],
  prismaModel: M
) {
...
async function import_images(dataset: ImageData[]) {
    ...
    const images = await prisma.image.findMany({
        where: { fullPath: { in: uniqueImageRecords.map(({ fullPath }) => fullPath) } },
        select: { id: true, fullPath: true }
    });
    const imageIdMap: Map<string, number> = new Map(images.map(({ id, fullPath }) => [fullPath, id]));
    ...
    const tagIdMap: Map<string, number> = new Map(tags.map(({ id, name }) => [name, id]));
    ...
    for (let i = 0; i < datasetChunks.length; i++) {
        ...
        if (imageTagRecords.length === 0) {
            console.warn(`No ImageTags records to insert for chunk ${i + 1}/${datasetChunks.length}.`);
        } else {
          try {
            await prisma.$transaction(async (transaction) => {
        ...
        console.log(`Processed ${totalProcessed}/${dataset.length} images (${((totalProcessed / dataset.length) * 100).toFixed(1)}%)`);
    }
