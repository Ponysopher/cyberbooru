...
import * as uploadModule from '@/lib/image-processing/process-upload';
...
vi.spyOn(uploadModule, 'process_upload');

describe('/api/upload', () => {
  it('accepts multipart/form-data', async () => {
    (uploadModule.process_upload as any).mockResolvedValue(mockImages);
  ...
  it('returns created image metadata', async () => {
    ...
    await testApiHandlerGeneric(async ({ fetch }) => {
      ...
      const expected = mockImages.map((img) => ({
        ...img,
